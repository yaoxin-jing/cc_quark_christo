...
use crate::{amd64_def::{SegmentDescriptor, SEGMENT_DESCRIPTOR_ACCESS, SEGMENT_DESCRIPTOR_EXECUTE,
    SEGMENT_DESCRIPTOR_PRESENT, SEGMENT_DESCRIPTOR_WRITE}, arch::{tee::NonConf, VirtCpu},
    kvm_vcpu::{AlignedAllocate, KVMVcpu, KVMVcpuState, SetExitSignal},
    qlib::{self, backtracer, common::{CR0_AM, CR0_ET, CR0_NE, CR0_PE, CR0_PG, CR4_FSGSBASE, CR4_OSFXSR,
    CR4_OSXMMEXCPT, CR4_OSXSAVE, CR4_PAE, CR4_PGE, CR4_PSE, EFER_LMA, EFER_LME, EFER_NX, EFER_SCE,
    KCODE, KDATA, KERNEL_FLAGS_SET, TSS, UDATA}, kernel::asm::xgetbv, linux_def::SysErr}, VMS};
use crate::{SHARE_SPACE, KERNEL_IO_THREAD, syncmgr::SyncMgr};
use crate::runc::runtime::vm;
use crate::arch::ConfCompExtension;
use crate::arch::ConfCompType;
use crate::GLOCK;
...
impl VirtCpu for X86_64VirtCpu {
    fn new_vcpu(vcpu_id: usize, total_vcpus: usize, vm_fd: &VmFd, entry_addr: u64,
        ...
        auto_start: bool, stack_size: usize, kvm: Option<&Kvm>, conf_extension: ConfCompType)
        -> Result<Self, Error> {
        if kvm.is_none() {
        ...
        let _gtd_addr = AlignedAllocate(MemoryDef::PAGE_SIZE as usize, MemoryDef::PAGE_SIZE as usize, true)?;
        let _idt_addr = AlignedAllocate(MemoryDef::PAGE_SIZE as usize, MemoryDef::PAGE_SIZE as usize, true)?;
        let _tisa = AlignedAllocate(MemoryDef::PAGE_SIZE as usize, MemoryDef::PAGE_SIZE as usize, true)?;
        let _tss_addr = AlignedAllocate(MemoryDef::PAGE_SIZE as usize, MemoryDef::PAGE_SIZE as usize, true)?;
        let _vcpu_base = KVMVcpu::Init(vcpu_id, total_vcpus, entry_addr, stack_size, _vcpu_fd, auto_start)?;

        info!("The tssIntStackStart:{:#x}, tss address:{:#x}, idt addr:{:#x}, gdt addr:{:#x}",
            _tisa, _tss_addr, _idt_addr, _gtd_addr);
        let _conf_type = crate::arch::ConfCompType::NoConf;
        let _conf_comp_ext = match conf_extension {
            ConfCompType::NoConf =>
                NonConf::initialize_conf_extension(share_space_table_addr, page_allocator_base_addr)?,
            _ => {
                return Err(Error::InvalidArgument("Create vcpu failed - bad ConfCompType".to_string()));
            }
        };
    ...
    fn default_hypercall_handler(&self, hypercall: u16, data: &[u8], arg0: u64, arg1: u64,
        ...
        //NOTE: Maybe not needed -> HYPERCALL_U64
        static mut FIRST: bool = true;
        static mut LAST_VAL: u32 = 0;
        ...
        match hypercall {
            ...
            qlib::HYPERCALL_U64 => unsafe {
                ...
                if FIRST {
                    ...
                    info!("HypercallU64 : 0x{:x}{:x}", LAST_VAL, val);
                    FIRST = true;
                }
            },
...
impl X86_64VirtCpu {
    ...
    fn setup_gdt(&self, sregs: &mut kvm_sregs) -> Result<(), Error> {
        let gdtTbl = unsafe {
            std::slice::from_raw_parts_mut(self.gtd_addr as *mut u64,
        ...
        let tssSegment = self.tss_addr as *mut x86_64::structures::tss::TaskStateSegment;
        unsafe {
    ...
    fn tss_descriptor(tss: &x86_64::structures::tss::TaskStateSegment) -> (u64, u64, u16) {
        let (tssBase, tssLimit) = Self::set_tss(tss);
    ...
    fn _run(&self) -> Result<(), Error> {
        ...
        loop {
            ...
            let kvm_ret = match self.vcpu_base.vcpu_fd.run() {
                ...
                Err(e) => {
                    if e.errno() == SysErr::EINTR {
                        ...
                        error!("vCPU[{}] - sregs:{:#x?}, error:{:#?}", self.vcpu_base.id, sregs, e);
            ...
            if let VcpuExit::IoOut(addr, data) = kvm_ret {
                {
                    let mut interrupting = self.vcpu_base.interrupting.lock();
                    interrupting.0 = false;
                    interrupting.1.clear();
