...
use libc::gettid;
use crate::{arch::{tee::NonConf, ConfCompExtension, ConfCompType, VirtCpu}, kvm_vcpu::{KVMVcpuState, SetExitSignal}, qlib::{self, common::Error,
            linux::time::Timespec, linux_def::{MemoryDef, SysErr}, qmsg::qcall::{Print, QMsg},
            GetTimeCall, VcpuFeq}, runc::runtime::vm, syncmgr::SyncMgr, KVMVcpu, GLOCK, KERNEL_IO_THREAD, SHARE_SPACE, VMS};
        ...
        loop {
            ...
            if let VcpuExit::MmioWrite(addr, data) = kvm_ret {
                ...
                if self.conf_comp_extension.should_handle_hypercall(hypercall) {
                    exit_loop = self.conf_comp_extension.handle_hypercall(hypercall, data, arg0, arg1,
                        arg2, arg3, self.vcpu_base.id)
                        .expect("VM run failed - cannot handle hypercall correctly.");
                } else {
                    exit_loop = self.default_hypercall_handler(hypercall, data, arg0, arg1, arg2, arg3)
                        .expect("VM run failed - cannot handle hypercall correctly.");
                }
