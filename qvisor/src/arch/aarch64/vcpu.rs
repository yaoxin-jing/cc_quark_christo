...
use crate::{arch::{tee::{NonConf, emulcc::EmulCc}, ConfCompExtension, VirtCpu},
            ...
            GetTimeCall, VcpuFeq, config::CCMode, kernel::arch::tee::get_tee_type}, runc::runtime::vm, syncmgr::SyncMgr, KVMVcpu, GLOCK,
...
impl VirtCpu for Aarch64VirtCpu {
    ...
    fn initialize_sys_registers(&self) -> Result<(), Error> {
        ...
        if get_tee_type() != CCMode::Cca {
            ...
            return self.conf_comp_extension.set_sys_registers(&self.vcpu_base.vcpu_fd, Some(reg_list));
        }
    }
