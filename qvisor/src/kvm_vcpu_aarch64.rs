...
//            let kvmRet = match self.vcpu_fd.run() {
//                Ok(ret) => ret,
//                Err(e) => {
...
//                        self.dump()?;
//                        if self.vcpu_fd.get_ready_for_interrupt_injection() > 0 {
...
//                    // x2,x3,x4,x5
//                    let para1 = self.vcpu_fd.get_one_reg(_KVM_ARM64_REGS_R2).map_err(|e| Error::SysError(e.errno()))?;
//                    let para2 = self.vcpu_fd.get_one_reg(_KVM_ARM64_REGS_R3).map_err(|e| Error::SysError(e.errno()))?;
//                    let para3 = self.vcpu_fd.get_one_reg(_KVM_ARM64_REGS_R4).map_err(|e| Error::SysError(e.errno()))?;
//                    let para4 = self.vcpu_fd.get_one_reg(_KVM_ARM64_REGS_R5).map_err(|e| Error::SysError(e.errno()))?;
...
//                    self.InterruptGuest();
//                    self.vcpu_fd.set_kvm_request_interrupt_window(0);
//                    {
//                        let mut interrupting = self.interrupting.lock();
//                        interrupting.0 = false;
//                        interrupting.1.clear();
...
//                    self.vcpu_fd.set_kvm_request_interrupt_window(1);
//                    {
//                        let mut interrupting = self.interrupting.lock();
//                        interrupting.0 = false;
//                        interrupting.1.clear();
