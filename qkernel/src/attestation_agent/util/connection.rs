...
use crate::syscalls::sys_file::SysClose;
use crate::{MonotonicNow, SyscallArguments, GUEST_HOST_SHARED_ALLOCATOR};
...
impl Connector {
    ...
    pub fn close(&self) -> Result<i64> {
        ...
        let fop = self.socket_file.FileOp.clone();
        let task = Task::Current();
        let arg = match fop {
            FileOps::HostSocketOperations(s) => {
                SyscallArguments {
                    arg0: s.fd as u64,
                    arg1: 0,
                    arg2: 0,
            ...
            _ => panic!("VM: expected HostSocketOperations type")
        };
        SysClose(task, &arg)
    }
