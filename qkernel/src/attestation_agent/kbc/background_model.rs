...
use crate::attestation_agent::util::connection::{
    ConnectionClient, HttpReq, KbsResponse, RespType,
};
...
impl KbsClientT for KbsClient<BackgroundCkeck> {
    ...
    fn get_token(
        ...
        if resp.is_err() {
            ...
            return core::prelude::v1::Err(resp.unwrap_err());
        } else {
    ...
    fn get_resource(
        ...
        if kbs_resp_res.is_err() {
            return core::prelude::v1::Err(kbs_resp_res.clone().unwrap_err());
        }
        let kbs_resp: KbsResponse = kbs_resp_res.unwrap();
        let resource: kbs_types::Response = kbs_resp.resource.unwrap();
        let plaintext: Result<Vec<u8>> = self.decrypt_resource(resource, self.tee_key.clone());
        if plaintext.is_err() {
...
impl<BackgroundCkeck> KbsClient<BackgroundCkeck> {
        ...
        loop {
            if retry < Self::KBS_RCAR_RETRY {
                ...
                match _res {
                    ...
                    Err(e) => match e {
                        Error::IOError(s) => {
                            if s.contains("TlsSockRead")
                                || s.contains("TlsSockSend")
                                || s.contains("TlsConn")
                            {
    ...
    fn rcar_handshake(
        &self,
        http_client: &mut ConnectionClient,
        tee: String,
        aa: &AttestationAgent,
    ) -> Result<Token> {
        debug!("VM: Do RCAR handshake");
        let tee_kp = self.tee_key.clone().expect("KBC key for TEE");
        let pub_tkey: TeePubKey = tee_kp.export_tee_pub_key();
        let mut req_type = HttpReq::Post(self.request_challenge());
        let challenge_request = ConnectionClient::build_request(
            tee,
            self.kbs_version.clone(),
            "".to_string(),
            &req_type,
        );
        ...
        if challenge_req_res.is_err() {
            ...
            return core::prelude::v1::Err(challenge_req_res.clone().unwrap_err());
        }
        ...
        if response_res.is_err() {
            ...
            return core::prelude::v1::Err(response_res.clone().unwrap_err());
        }
        ...
        let hushed_data = self
            .hash_data(pub_tkey.clone(), response)
        ...
        let att_report = ConnectionClient::build_attest_report(pub_tkey, hw_meas, &req_type);
        let att_rep_res = http_client.send_request(att_report);
        if att_rep_res.is_err() {
            debug!("VM: RCAR handshake failed - talking to KBS failed.");
            return core::prelude::v1::Err(att_rep_res.clone().unwrap_err());
        }
        ...
        if att_resp_res.is_err() {
            ...
            return core::prelude::v1::Err(att_resp_res.clone().unwrap_err());
        }
        let att_report: KbsResponse = att_resp_res.unwrap();
        let token = Token::new(att_report.token.unwrap()).expect("Invalid token");
        Ok(token)
    }
    ...
    fn hash_data(&self, pub_tkey: TeePubKey, to_hash_data: KbsResponse) -> Result<Vec<u8>> {
        ...
        if let Some(hash_code) = &to_hash_data.extra_params {
            hashed_data = if hash_code.to_uppercase().contains(Self::KBS_HASH) {
                ...
                Self::hash_data_sha384(pub_tkey, to_hash_data.nonce.unwrap(), None)
            } else {
                panic!("VM: KBS requested unsupported hash-code:{:?}", hash_code)
            };
        } else {
            // Default
            hashed_data = Self::hash_data_sha384(pub_tkey, to_hash_data.nonce.unwrap(), None);
        }
        hashed_data
    }
    ...
    fn hash_data_sha512(
        pub_key: TeePubKey,
        nonce: String,
        runtime_measurement: Option<String>,
    ) -> Result<Vec<u8>> {
        use sha2::{Digest, Sha512};
        debug!(
            "VM: Hash runtime - Nonce:{:?}, Nonce-length:{}",
            nonce,
            nonce.len()
        );
        ...
        let binding = serde_json::to_string(&challenge).unwrap();
