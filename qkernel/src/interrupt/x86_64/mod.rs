...
cfg_snp! {
    ...
    use crate::qlib::kernel::arch::tee::sev_snp::{VCResult,SVMExitDef};
    use crate::qlib::kernel::arch::tee::sev_snp::ghcb::*;
    use alloc::slice;
    use crate::qlib::kernel::arch::tee::sev_snp::vc::*;
...
pub extern "C" fn PageFaultHandler(ptRegs: &mut PtRegs, errorCode: u64) {
    ...
    #[cfg(feature = "snp")]
    {
        if tee::get_tee_type() == CCMode::SevSnp {
            cr2 = tee::guest_physical_address(cr2);
        }
    }
...
pub extern "C" fn VmmCommunicationHandler(sf: &mut PtRegs, errorCode: u64) {
    use crate::Kernel::LOG_AVAILABLE;
