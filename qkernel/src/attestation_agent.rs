...
impl AttestationAgent {
    pub fn try_attest(config_path: Option<String>, envv: Option<Vec<String>>) {
        ...
        for item in resource_list {
            ...
            retrived_resource.push((item.0, resource));
        }
    ...
    fn install_resource(list: Vec<(String, Vec<u8>)>) {
        ...
        for (name, content) in list {
            let fname = format!("/opt/{}",name);
            let addr = fname.as_ptr() as u64;
            let fd = createAt(task, ATType::AT_FDCWD,
                addr, flag as u32, FileMode(mode)).expect("crate failed");
            if fd > 0i32 {
                let size: i64 = content.len().try_into().unwrap();
                let addr = content.as_slice().as_ptr() as u64;
                ...
                panic!("VM: AA - failed to create :{:?} on guest", fname);
            }
        }
    }
