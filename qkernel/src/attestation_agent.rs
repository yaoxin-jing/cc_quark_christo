...
impl AttestationAgent {
    pub fn try_attest(config_path: Option<String>, envv: Option<Vec<String>>) {
        ...
        let mut rng = OsRng;
       // let bind = aa.kbc.create_connect_client();
       // aa.kbc.as_mut().connect_to_kbs(&bind, &mut read_rec, &mut write_rec, &mut rng);
        let httpc = HttpSClient::create_http_client(aa.kbc.kbs_address(),
            kbc::KBC_KEY_LENGTH, String::from("RSA1_5")/*kbc::KBC_ENC_ALG*/);
        let bind = httpc.clone();
        let mut rng = OsRng;
        let tls = tls_connection(&bind, &mut read_rec, &mut write_rec, &mut rng)
            .map_err(|e| {
...
impl AttestationAgentT for AttestationAgent {
    fn get_attestation_token(&mut self, con_client: &mut ConnectionClient) -> Result<AttestationToken> {
        ...
        let (token, tkp/*, httpc*/) = self.kbc.get_token(tee, con_client, &self)
            .expect("AA - failed to get token");
